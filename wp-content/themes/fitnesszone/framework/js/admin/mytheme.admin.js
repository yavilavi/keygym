...
var mythemeAdmin = {
  init : function(){
    ...
    mythemeAdmin.addItem();
    mythemeAdmin.removeItem();
  ...
  menuTeamAdd : function(){
    "use strict";
    jQuery('.mytheme_team_add_item').click(function(e){
      var $this = jQuery(this).parent().next(),
          $appendTo = $this.find('.menu-to-edit-team'),
          $itemToClone = $this.find('.sample-to-edit li'),
          $item = $appendTo.find('li'),
          $itemsCount = $item.length,
          $allIds = []; //$allIds = new Array();
		...
		$ptemplate_select.live('change', function(){
			...
			switch($val){
				case 'tpl-blog.php':
					...
					$ptemplate_box.find('#tpl-home-fullwidth-settings').slideUp();
				...
				case 'tpl-home.php':
					...
					$ptemplate_box.find('#tpl-common-settings').slideDown();
					$ptemplate_box.find('#tpl-default-settings').slideDown();
					$ptemplate_box.find('#tpl-home-fullwidth-settings').slideDown();
					jQuery("#page-template-slider-meta-container").slideDown();
  ...
  addItem : function() {
    jQuery(".add-item").click(function(event) {
      $parent = jQuery(this).parents(".cform-question");
      $clone = jQuery(this).siblings(".clone").find(".general-cform-container").clone();
      console.log( $clone );
  ...
  removeItem : function(){
    jQuery("body").delegate(".remove-item","click",function(event){
      jQuery(this).parents(".general-cform-container").remove();
      event.preventDefault();
    });
  }, //RemoveItem
