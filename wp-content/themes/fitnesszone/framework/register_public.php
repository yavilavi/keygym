...
if( !function_exists('dt_theme_wp_list_categories') ) {
...
if( !function_exists('dt_theme_wp_list_archive') ) {
...
if( !function_exists('dt_theme_filter_widget_product_categories') ) {
...
class DT_Theme_Walker_Category extends Walker_Category {
   function start_el(&$output, $category, $depth = 0, $args = array(), $current_category = 0) {
      ...
      $link = '<a href="' . get_term_link( $category->term_id, 'product_cat' ) . '" ';
      if ( $use_desc_for_title == 0 || empty($category->description) )
         $link .= 'title="' . sprintf(__( 'View all products under %s', 'fitnesszone' ), $cat_name) . '"';
      else
 ...
 **/
if( !function_exists('dt_theme_default_navigation') ) {
 ...
 **/
if( !function_exists('dt_theme_footer_navigation') ) {
...
if( !function_exists('dt_theme_enqueue_scripts') ) {
	function dt_theme_enqueue_scripts() {
	
		$template_uri = get_template_directory_uri().'/framework';
	
		wp_enqueue_script('jq-html5', $template_uri.'/js/public/html5shiv.min.js', array(), '3.7.2', true);
		wp_script_add_data('jq-html5', 'conditional', 'lt IE 9');
	
		wp_enqueue_script('jq-canvas', $template_uri.'/js/public/excanvas.js', array(), '2.0', true);
		...
		echo "\t\t theme_base_url:'".esc_js(FITNESSZONE_BASE_URL)."'";
		...
		$cookiebar = dt_theme_option('privacy', 'enable-cookie-msgbar');
		if( isset($cookiebar) ) {
			wp_enqueue_script('jq-cookie-js', $template_uri.'/js/public/cookieconsent.js', array(), false, true);
		}

		wp_enqueue_script('jq-popup-js', $template_uri.'/js/public/magnific/jquery.magnific-popup.min.js', array(), false, true);
	}
}
...
if( !function_exists('dt_theme_seo_meta') ) {
 ...
 **/
if( !function_exists('dt_theme_appearance_load_fonts') ) {
 ...
 **/
if( !function_exists('dt_theme_appearance_css') ) {
	function dt_theme_appearance_css() {
		...
		if (dt_theme_option("appearance", "layout") == "boxed") :
			...
			if (dt_theme_option("appearance", "bg-type") == "bg-patterns") :
				...
				if (!empty($pattern)) {
					$output .= "background-image:url('".FITNESSZONE_FW_URL."theme_options/images/patterns/{$pattern}');";
				}
		...
		if (empty($disable_styling)) :
			...
			$styling_site_title_color = dt_theme_option('appearance','styling-site-title-color');
			...
			if(!empty($styling_site_title_color) and $styling_site_title_color != "#")
				$output .= ".logo-title h1 a, .logo-title h2 { color: $styling_site_title_color; }" ;
 ...
 **/
if( !function_exists('dt_theme_slider_section') ) {
 ...
 **/
if( !function_exists('dt_theme_excerpt') ) {
 ...
 **/
if( !function_exists('dt_theme_custom_comments') ) {
	function dt_theme_custom_comments($comment, $args, $depth) {
		...
		switch ( $comment->comment_type ) :
		   ...
		   case 'trackback' :
				...
				echo "<p>";
				  _e( 'Pingback:','fitnesszone');
				  comment_author_link();
				  edit_comment_link( __('Edit','fitnesszone'), ' ' ,'');
				echo "</p>";
		   break;
				...
				echo '<article class="comment">';
					...
					echo '<section class="comment-details">';
						...
						echo '<div class="commentmetadata">';
							printf(__( '%1$s at %2$s', 'fitnesszone'), get_comment_date('d M Y'), get_comment_time());	
						echo '</div>';
	
						if($comment->comment_approved == '0'):
						  echo '<p>'.__( 'Your comment is awaiting moderation.','fitnesszone').'</p>';
						endif;
						...
						echo '<div class="reply">';
							echo comment_reply_link( array_merge( $args, array('reply_text'=>__('Reply','fitnesszone'), 'depth' => $depth, 'max_depth' => $args['max_depth'] ) ) );
						echo '</div>';
...
class dt_theme_breadcrumb {
	...
	function __construct() {
		...
		echo '<div class="breadcrumb"><a href="'.home_url().'">'.__('Home', 'fitnesszone').'</a>';
	...
	function simple_breadcrumb_case($der_post) {
			...
			if (is_attachment()) {
				...
				if ($postType == 'post') {
					...
					if ($postType == 'events') {
						...
						if ($postType == 'forum') {
							global $post;
							echo '<a href="'.home_url().'/forums/">'.__('Forums', 'fitnesszone').'</a>'.$markup;
					} else
						if ($postType == 'topic') {
							global $post;
							echo '<a href="'.get_permalink($post->post_parent).'">'.__('Forum','fitnesszone').'</a>'.$markup;
					}
			}
			return;
		}
		...
		if (is_category()) {
			...
			return __("Archive for Category: ", 'fitnesszone').single_cat_title('', FALSE);
		}

		if (is_author()) {
			$curauth = get_user_by('login', get_query_var('author_name'));
			return __("Archive for Author: ", 'fitnesszone').$curauth->nickname;
		}

		if (is_tag()) {
			return __("Archive for Tag: ", 'fitnesszone').single_tag_title('', FALSE);
		}

		if (is_404()) {
			return __("LOST", 'fitnesszone');
		}

		if (is_search()) {
			return __("Search", 'fitnesszone');
		}
		...
		if (is_post_type_archive('product')) {
			return __("Products", 'fitnesszone');
		}

		if (is_post_type_archive('forum')) {
			return __("Forums", 'fitnesszone');
		}
		
		if(in_array('events-archive', get_body_class())) {
			return __('Events', 'fitnesszone');
		}

		if(in_array('topic-tag', get_body_class()))  {
...
if( !function_exists('dt_theme_color_picker') ) {
	function dt_theme_color_picker(){
	
		$patterns_url = FITNESSZONE_FW_URL."theme_options/images/pattern/";
		$skins_url = FITNESSZONE_BASE_URL."images/style-picker/";
		
		$patterns = "";
		$patterns_array =  dt_theme_listImage(get_template_directory()."/images/style-picker/patterns/");
		
		foreach($patterns_array as $k => $v){
			$img = 	FITNESSZONE_BASE_URL."images/style-picker/patterns/".$k;
			$patterns .= '<li>';
		...
		$str .= '	<a href="" title="" class="style-picker-ico"> <img src="'.FITNESSZONE_BASE_URL.'images/style-picker/picker-icon.png" alt="" title="" /> </a>';
		$str .= '	<div id="dt-style-picker">';
		$str .= '   	<h2>'.__('Select Your Style','fitnesszone').'</h2>';
		
		$str .= '       <h3>'.__('Choose your layout','fitnesszone').'</h3>';
		$str .= '		<ul class="layout-picker">';
		$str .= '       	<li> <a id="fullwidth" href="" title="" class="selected"> <img src="'.FITNESSZONE_BASE_URL.'images/style-picker/fullwidth.jpg" alt="" title="" /> </a> </li>';
		$str .= '       	<li> <a id="boxed" href="" title=""> <img src="'.FITNESSZONE_BASE_URL.'images/style-picker/boxed.jpg" alt="" title="" /> </a> </li>';
		$str .= '		</ul>';
		$str .= '		<div class="hr"> </div>';
		$str .= '		<div id="pattern-holder" style="display:none;">';
		$str .='			<h3>'.__('Patterns for Boxed Layout','fitnesszone').'</h3>';
		...
		$str .= '		<h3>'.__('Color scheme','fitnesszone').'</h3>';
