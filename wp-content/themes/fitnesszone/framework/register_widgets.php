...
$mailchimp_file_path = FITNESSZONE_CORE_PLUGIN.'/apis/mailchimp/mailchimp.php';
if( file_exists( $mailchimp_file_path ) ){
	require_once FITNESSZONE_CORE_PLUGIN.'/apis/mailchimp/mailchimp.php';
}
...
if( !function_exists('dt_style_widget_form') ) {
	function dt_style_widget_form( $widget, $return, $instance ){?>
		<p>
			<label for="<?php echo $widget->get_field_id('style'); ?>"><?php _e('Font Icon Class (eg: fa-home):','fitnesszone');?>
...
if( !function_exists('dt_update_widget_form') ) {
...
if( !function_exists('dt_add_widget_styles') ) {
