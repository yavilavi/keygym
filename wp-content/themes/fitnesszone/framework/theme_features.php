<?php if (!function_exists('dt_theme_features')) {
	...
	function dt_theme_features() {
		...
		load_theme_textdomain('fitnesszone', get_template_directory().'/languages');
...
if (!function_exists('dt_theme_navigation_menus')) {
	...
	function dt_theme_navigation_menus() {
		$locations = array(
			'primary-menu' => __('Primary Menu', 'fitnesszone'),
			'secondary-menu' => __('Secondary Menu', 'fitnesszone')
		);
...
if (!function_exists( 'dt_theme_admin_fonts_url')) {
	...
	function dt_theme_admin_fonts_url() {
		...
		if ( 'off' !== _x( 'on', 'Noto Sans font: on or off', 'fitnesszone' ) ) {
			$fonts[] = 'Noto Sans:400italic,700italic,400,700';
		}
	
		/* translators: If there are characters in your language that are not supported by Noto Serif, translate this to 'off'. Do not translate into your own language. */
		if ( 'off' !== _x( 'on', 'Noto Serif font: on or off', 'fitnesszone' ) ) {
			$fonts[] = 'Noto Serif:400italic,700italic,400,700';
		}
	
		/* translators: If there are characters in your language that are not supported by Inconsolata, translate this to 'off'. Do not translate into your own language. */
		if ( 'off' !== _x( 'on', 'Inconsolata font: on or off', 'fitnesszone' ) ) {
			$fonts[] = 'Inconsolata:400,700';
		}
	
		/* translators: To add an additional character subset specific to your language, translate this to 'greek', 'cyrillic', 'devanagari' or 'vietnamese'. Do not translate into your own language. */
		$subset = _x( 'no-subset', 'Add new subset (greek, cyrillic, devanagari, vietnamese)', 'fitnesszone' );
...
// Filter to modify title tag...
add_filter( 'pre_get_document_title', 'dt_theme_pre_get_document_title', 999, 1);
if( !function_exists('dt_theme_pre_get_document_title') ) {
	function dt_theme_pre_get_document_title( $title ){
		$title = dt_theme_public_title();
