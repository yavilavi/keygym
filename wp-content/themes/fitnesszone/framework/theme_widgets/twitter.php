...
class MY_Twitter extends WP_Widget {
	...
	function __construct() {
		...
		parent::__construct(false,FITNESSZONE_THEME_NAME.__(' Twitter Widget','fitnesszone'),$widget_options);
	}

	#2.widget input form in back-end
	function form($instance) {
		$instance = wp_parse_args( (array) $instance,array( 'title' => __('Latest Tweets','fitnesszone'), 'count' => '3', 'username' => '',
        ...
        <p><label for="<?php echo $this->get_field_id('consumer_key'); ?>"><?php _e('Consumer Key:','fitnesszone');?> 
        ...
        <p><label for="<?php echo $this->get_field_id('consumer_secret'); ?>"><?php _e('Consumer Secret:','fitnesszone');?> 
        ...
        <p><label for="<?php echo $this->get_field_id('access_token'); ?>"><?php _e('Access Token:','fitnesszone');?> 
        ...
        <p><label for="<?php echo $this->get_field_id('access_token_secret'); ?>"><?php _e('Access Token Secret:','fitnesszone');?> 
        ...
        <p><label for="<?php echo $this->get_field_id('username'); ?>"><?php _e('Enter your twitter username:','fitnesszone');?>
        ...
        <p><label for="<?php echo $this->get_field_id('count'); ?>"><?php _e('How many entries do you want to show:','fitnesszone');?>
        ...
        <p><input type="checkbox"  id="<?php echo $this->get_field_id('exclude_replies');?>" name="<?php echo $this->get_field_name('exclude_replies');?>" 
			<?php checked($exclude_replies); ?> /> <label for="<?php echo $this->get_field_id('exclude_replies'); ?>"><?php _e('Exclude @replies','fitnesszone');?></label></p>
            
        <p><input type="checkbox"  id="<?php echo $this->get_field_id('time');?>" name="<?php echo $this->get_field_name('time');?>" 
			<?php checked($time); ?> /> <label for="<?php echo $this->get_field_id('time'); ?>"><?php _e('Show time of tweet','fitnesszone');?></label></p>
            
        <p><input type="checkbox"  id="<?php echo $this->get_field_id('time');?>" name="<?php echo $this->get_field_name('display_avatar');?>" 
				<?php checked($display_avatar); ?> /> <label for="<?php echo $this->get_field_id('display_avatar'); ?>"><?php _e('Show user avatar','fitnesszone');?></label></p>		
<?php
	}
	...
	function widget($args, $instance) {
		...
		echo $before_widget;			
			...
			if($username && $consumer_key && $consumer_secret && $access_token && $access_token_secret && $count) { 
					...
					$twitter_file_path = FITNESSZONE_CORE_PLUGIN."/apis/twitteroauth/twitteroauth.php";
					if( file_exists( $twitter_file_path ) ) {
						require_once FITNESSZONE_CORE_PLUGIN."/apis/twitteroauth/twitteroauth.php";
					}
				...
				if($twitter && is_array($twitter)) {
					...
					echo '<li>'.__('No public Tweets found','fitnesszone').'</li>';
				}
				echo "</ul>";
			}
		echo $after_widget;
