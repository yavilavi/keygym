...
class MY_Recent_Posts extends WP_Widget {
	...
	function __construct() {
		...
		parent::__construct(false,FITNESSZONE_THEME_NAME.__(' Posts','fitnesszone'),$widget_options);
	}
	...
	function form($instance) {
	    ...
	    <p><label for="<?php echo $this->get_field_id('_post_categories'); ?>">
			<?php _e('Choose the categories you want to display (multiple selection possible)','fitnesszone');?></label>
        ...
        <p><label for="<?php echo $this->get_field_id('_excerpt'); ?>"><?php _e('Display title only or title &amp; excerpt','fitnesszone');?></label>
        ...
        <p><input type="checkbox"  id="<?php echo $this->get_field_id('_enabled_image');?>" name="<?php echo $this->get_field_name('_enabled_image');?>"
	         <?php checked($_enabled_image); ?> /> <?php _e("Show Image",'fitnesszone');?></p>  

	    <p><label for="<?php echo $this->get_field_id('_post_count'); ?>"><?php _e('No.of posts to show:','fitnesszone');?></label>
	...
	function widget($args, $instance) {
		...
		echo "<div class='recent-posts-widget'><ul>";
			 ...
			 while($the_query->have_posts()):
				...
				echo "<li>";
					if(1 == $_enabled_image):
						$image = wp_get_attachment_image_src(get_post_thumbnail_id($tid), 'my-post-thumb', false);
						$image = ( $image != false)? $image[0] : "http".dt_theme_ssl()."://placehold.it/100x80";
						echo "<a href='".get_permalink($tid)."' class='thumb'>";
						echo "<img src='$image' alt='{$title}'/>";
						echo "</a>";
					endif;

					if($show_title):
						echo "<h4><a href='".get_permalink($tid)."'>{$title}</a></h4>";
					else:
						echo "<h4><a href='".get_permalink($tid)."'>{$title}</a></h4>";
						$p = get_post($tid, ARRAY_A);
						$excerpt = $p['post_content'];
						$excerpt = wp_trim_words( $excerpt, 7, ' [&hellip;]' );
						echo ($excerpt);
					endif;

					echo '<div class="entry-metadata">';
						echo '<p class="author"> <span class="fa fa-user"> </span> '.__('Posted By: ', 'fitnesszone').'<a href="'.get_author_posts_url(get_the_author_meta('ID')).'">'.get_the_author_meta('display_name').'</a></p>';
						$commtext = "";
						if((wp_count_comments($tid)->approved) == 0)	$commtext = '0';
						else $commtext = wp_count_comments($tid)->approved;
						echo '<p> <a href="'.get_permalink($tid).'#comments"><span class="fa fa-comment"> </span>'.$commtext.'</a></p>';
					echo '</div>';
				echo "</li>";
			 endwhile;
			 wp_reset_postdata();			 
			 else:
			 	echo "<li><h4>".__('No Posts found','fitnesszone')."</h4></li>";
			 endif;
	 	echo "</ul></div>";
		echo $after_widget;
	}
