...
class MY_Upcoming_Events extends WP_Widget {
	...
	function __construct() {
		...
		parent::__construct(false,FITNESSZONE_THEME_NAME.__(' Events','fitnesszone'),$widget_options);
	}
			 ...
			 while($the_query->have_posts()):
				...
				echo "<li>";
					...
					echo '<div class="entry-metadata">';
						echo '<p class="date">'.__('Start time: ', 'fitnesszone').substr(get_post_meta($post->ID, '_EventStartDate', true), 0, 16).'<br>'.__('End time: ', 'fitnesszone').substr(get_post_meta($post->ID, '_EventEndDate', true), 0, 16).'</p>';
					echo '</div>';
				echo "</li>";
			 endwhile;
			 else:
			 	echo "<li><h4>".__('No Events found','fitnesszone')."</h4></li>";
			 endif;
			 wp_reset_postdata();
	 	echo "</ul></div>";			 
		echo $after_widget;
