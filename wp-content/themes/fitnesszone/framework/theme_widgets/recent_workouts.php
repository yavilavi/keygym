...
class MY_Recent_Workouts extends WP_Widget {
	...
	function __construct() {
		...
		parent::__construct(false,FITNESSZONE_THEME_NAME.__(' Workouts','fitnesszone'),$widget_options);
	}
				...
				echo '<div class="dt-excersise-entry">';
					echo "<div class='dt-excersise-title title'>";
						...
						if(!empty($wmeta['nosteps'])):
							echo '<p class="count">';
								echo "<a href='javascript:void(0)'>".$wmeta['nosteps']." <br><span>".__('Steps', 'fitnesszone')."</span></a>";
							echo '</p>';
						else:
					...
					if(!$show_title) {
						$p = get_post($post->ID, ARRAY_A);
						$excerpt = $p['post_content'];
						$excerpt = wp_trim_words( $excerpt, 10, ' [&hellip;]' );
						echo ($excerpt);
					}
				echo '</div>';
			 endwhile;
			 else:
			 	echo "<p>".__('No Workouts found','fitnesszone')."</p>";
			 endif;
			 wp_reset_postdata();
	 	echo "</div>";
		echo $after_widget;
