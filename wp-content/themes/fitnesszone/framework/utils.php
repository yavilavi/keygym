 ...
 *
 **/
if( !function_exists('dt_theme_public_title') ) {
	function dt_theme_public_title() {
		...
		if (!$status) :
			...
			// is_search()
			elseif (is_search ()) :
				$args ["search"] = __ ( "Search results for", 'fitnesszone' ) . ' "' . $_REQUEST ['s'] . '"'; // dding search text into the default args
			...
			elseif (is_404 ()) :
				...
				foreach ( $options as $option ) :
					if (array_key_exists ( $option, $args ))
						$doctitle .= $args [$option] . ' ' . $separator . ' ';
				endforeach;
				$doctitle = $doctitle . __ ( 'Page not found', 'fitnesszone' );
				$split = false;
			...
			elseif( ('tribe_events'== get_post_type() || is_singular('tribe_events') || is_singular('tribe_venue') || is_singular('tribe_organizer') || in_array('tribe-filter-live', get_body_class())) and !is_search()):			
				$doctitle = $args ["blog_title"] . ' ' . $separator . ' ' . __ ( 'Events', 'fitnesszone' );
				$split = false;
			endif;
 ...
 */
if( !function_exists('dt_theme_canonical') ) {
 ...
 */
if( !function_exists('dt_theme_footer_widgetarea') ) {
	function dt_theme_footer_widgetarea($count) {
		$name = __ ( "Footer Column", 'fitnesszone' );
		if ($count <= 4) :
			for($i = 1; $i <= $count; $i ++) :
 ...
 */
if( !function_exists('dt_theme_show_footer_widgetarea') ) {
 ...
 */
if( !function_exists('dt_theme_is_plugin_active') ) {
 ...
 */
if( !function_exists('check_slider_revolution_responsive_wordpress_plugin') ) {
 ...
 */
if( !function_exists('dt_theme_social_bookmarks') ) {
 ...
 */
if( !function_exists('is_dt_theme_moible_view') ) {
...
if( !function_exists('dt_theme_enqueue_styles') ) {
	function dt_theme_enqueue_styles() {
		...
		if( isset($cookie_bar) ) {
			wp_enqueue_style( 'trendy-cookie-css', $template_uri .'/css/cookieconsent.css' );
		}

		wp_enqueue_style( 'trendy-popup-css', $template_uri .'/framework/js/public/magnific/magnific-popup.css' );
...
// PAGINATION
// ##########################################
if( !function_exists('dt_theme_pagination') ) {
	function dt_theme_pagination($class='',$pages = '', $wp_query){
		...
		if($paged > 1) $out .= "<div class='prev-post'><a href='".get_pagenum_link($paged - 1)."'><span class='fa fa-angle-double-left'></span>&nbsp;".__('Prev', 'fitnesszone')."</a></div>";
		$out .= '<ul>';
			if(1 != $pages){
				for ($i=1; $i <= $pages; $i++){
		...
		$out .= '</ul>';
		if ($paged < $pages) $out .= "<div class='next-post'><a href='".get_pagenum_link($paged + 1)."'>".__('Next', 'fitnesszone')."&nbsp;<span class='fa fa-angle-double-right'></span></a></div>";
...
if( !function_exists('theme_chk_pp') ) {
...
if( !function_exists('dt_like_plugin_hook') ) {
...
if( !function_exists('slt_wmode_opaque') ) {
...
if( !function_exists('dt_theme_page_permalink_by_its_template') ) {
...
if( !function_exists('dt_theme_show_sidebar') ) {
...
if( !function_exists('dt_theme_event_venue_details') ) {
	function dt_theme_event_venue_details($event_ID) {
		...
		$out = '<p>'.$venue_all.'&nbsp; <a href="https://maps.google.com/maps?f=q&source=s_q&hl=en&geocode=&q='.urlencode($venue_all).'"><span class="fa fa-map-marker"></span> '.__('Google Map','fitnesszone').'</a></p>';
		return $out;
	}
}
...
# break oembeds.
if( !function_exists('dt_theme_undo_recurrence_oembed_logic') ) {
...
if( !function_exists('dt_theme_update_pagebuilder_contents') ) {
	function dt_theme_update_pagebuilder_contents() {
		...
		$bp_data = get_option(FITNESSZONE_THEME_SETTINGS);
		$bp_data['pagebuilder_update'] = 'done';
		update_option(FITNESSZONE_THEME_SETTINGS, $bp_data);
