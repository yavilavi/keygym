<?php get_header();
      ...
      <div class="breadcrumb-wrapper">
          <div class="container">
              <h1><?php if(is_category()): _e('Category Archives of : ', 'fitnesszone'); echo single_cat_title();
                          elseif(is_day()):  _e('Daily Archives of : ', 'fitnesszone'); echo get_the_date('l');
                          elseif(is_month()):  _e('Monthly Archives of : ', 'fitnesszone'); echo get_the_date('F, Y');
                          elseif(is_year()):  _e('Yearly Archives of : ', 'fitnesszone'); echo get_the_date('Y');
                          elseif(is_author()):  _e('Archive by Author : ', 'fitnesszone'); $author = get_user_by('login', get_query_var('author_name')); echo $author->nickname;
                          elseif(is_tag()):  _e('Tag Archives of : ', 'fitnesszone'); echo single_tag_title('', true);
                          elseif(taxonomy_exists('category')):  _e('Archive of : ', 'fitnesszone'); echo single_cat_title();
                          endif; ?></h1>
