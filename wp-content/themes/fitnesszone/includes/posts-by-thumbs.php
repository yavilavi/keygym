<?php
	...
	#Post layout check...
	$article_class = "dt-sc-one-column column blog-thumb";
	$feature_image = "blog-thumb";
	$column = "";
	...
	if($the_query->have_posts()): $i = 1;
	 echo '<div class="tpl-blog-holder">';
			...
			<article id="post-<?php the_ID(); ?>" <?php post_class('blog-entry'); ?>>
				<div class="blog-entry-inner">
					...
					<div class="entry-metadata"><?php
                    	if(is_sticky()): ?>
                          <div class="featured-post"><span><?php _e('Featured','fitnesszone'); ?></span></div><div class="clear"></div>
						...
						<a href="<?php the_permalink(); ?>"><?php _e('Read More', 'fitnesszone'); ?> <i class="fa fa-angle-double-right"></i></a>
					</div>
				</div>
			</article>
		</div><?php
	 ...
	 wp_reset_postdata();
		...
		<h2><?php _e('Nothing Found.', 'fitnesszone'); ?></h2>
		<p><?php _e('Apologies, but no results were found for the requested archive.', 'fitnesszone'); ?></p><?php
    endif; ?>