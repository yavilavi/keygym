...
class fitness_import {
	...
	function import_content( $file = 'all.xml.gz' ){
		...
		$xml = FITNESSZONE_FW.'importer/'.$file;

		$import->fetch_attachments = ( ($_REQUEST && key_exists('attachments', $_REQUEST['data']) && $_REQUEST['data']['attachments']) || ( $_REQUEST['data']['content'] == 'media' ) ) ? true : false;
	...
	function import_menu_location( $file = 'menu.txt' ){
		...
		$file_path 	= FITNESSZONE_FW_URL.'importer/'. $file;
	...
	function import(){
		...
		if( ! class_exists( 'WP_Import' ) ){
			require_once FITNESSZONE_CORE_PLUGIN . '/importer/wordpress-importer.php';
		}
		...
		if( class_exists( 'WP_Importer' ) && class_exists( 'WP_Import' ) ){
			...
			switch( $_REQUEST['data']['import'] ) {
				...
				case 'all':
					...
					$this->import_options( $import_demo.'/options.txt' );

					$this->import_content( $import_demo.'/content/posts.xml.gz' );
					$this->import_content( $import_demo.'/content/galleries.xml.gz' );
					$this->import_content( $import_demo.'/content/workouts.xml.gz' );
					$this->import_content( $import_demo.'/content/products.xml.gz' );
					$this->import_content( $import_demo.'/content/pages.xml.gz' );
					$this->import_content( $import_demo.'/content/media.xml.gz' );					
				...
				default:
					// Empty select.import
					$this->error = esc_html__('Please select data to import.','dt_themes');	
					break;
			}
			...
			if( $this->error ){
				...
				echo '<strong>'. esc_html__('All done. Have fun!','dt_themes') .'</strong>';
			}
		}
		die(0);
	}
...
$dtthemes_import = new fitness_import;
