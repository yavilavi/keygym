	...
	class DTPageBuilder {
		...
		function dt_add_custom_metabox(){
			...
			foreach ( $post_types as $post_type ){
				add_meta_box( 'dtthemes-metabox', $theme_name.esc_html__( ' Page Builder', 'dt_themes' ), array ($this, 'dtthemes_custom_metabox_layout'), $post_type, 'normal', 'high', array( 'id' => $ptid) );
			}
		...
		function dtthemes_modules_init(){
			...
			foreach($wp_widget_factory->widgets as $class => $info){
				...
				$dt_form = mb_convert_encoding($dt_form, 'HTML-ENTITIES', "UTF-8");
		...
		function dt_disable_builder_option(){
			...
			$pboptions = get_option(FITNESSZONE_THEME_SETTINGS);
			$pboption = isset($pboptions['pagebuilder']['post-types']) ? $pboptions['pagebuilder']['post-types'] : false;
			if(!isset($dt_builder_enable)) {
				if($pboption != false && $pboptions['pagebuilder']['enable-pagebuilder'] == true) $dt_builder_enable = 1;
			...
			echo '<div class="dt_builder_option">'
					. esc_html__( 'Enable page builder', 'dt_themes' ) .' <div data-for="dtthemes-enable-builder" id="dt_enable_builder" class="chkbx-switch '.$switch_cls.'"></div>' 
